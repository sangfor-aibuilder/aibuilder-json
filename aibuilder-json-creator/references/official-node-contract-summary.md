...
- File upload workflows may require a runtime `workflowStart.userFiles` output even though the standalone official default node only lists `userChatInput`; only add it when `chatConfig.fileSelectConfig.canSelectFile = true` and file parsing is part of the workflow.
